...
% \subsection{模型建立}
% 为解决各学院评分标准不一致、分布极差大的问题，本文构建了基于分层贝叶斯（Hierarchical Bayesian Model, HBM）的评分标准化校正模型。建模核心过程如下：

% \textbf{1. 模型假设}
% \begin{itemize}
%     \item 教师评分数据涵盖全校所有学院，部分学院下有多个专家组，教师-专家组-学院关系已知，数据完整无缺失或已补齐；
%     \item 各学院和专家组均存在固定但未知的系统性评分偏差（如评分偏松或偏紧），评分误差服从独立同分布正态噪声；
%     \item 教师真实教学水平存在客观差异，目标是还原、剥除非个人能力因素对分数的系统性干扰。
% \end{itemize}

% \textbf{2. 变量定义}
% \begin{itemize}
%     \item $S_{ijk}$：第$i$学院第$j$专家组第$k$教师的原始评价得分；
%     \item $\mu_{global}$：全校教师真实整体水平均值；
%     \item $\alpha_i$：第$i$学院的系统性评分偏差（学院偏效应）；
%     \item $\beta_{ij}$：第$i$学院第$j$专家组的系统性评分偏差（小组偏效应）；
%     \item $\epsilon_{ijk}$：独立误差项，$\epsilon_{ijk} \sim \mathcal{N}(0, \sigma_{error}^2)$。
% \end{itemize}
...
% \textbf{Step 4：方法解释与优势}
% \begin{itemize}
%     \item 该模型能自动校正系统性误差，体现分层收缩效应（小样本极端分数被全体均值牵引），不同规模学院均获得更公平评价；
%     \item 全流程可复现，可追踪异常组织（如偏紧/松专家组），有利于后续高校管理与优化；
%     \item 标准化分数便于历年/全校横向比较，实现科学、公正的汇总排名。
% \end{itemize}
